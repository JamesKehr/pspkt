...
The parser maintains **per-capture TID→tree-name and FID→file-name tables**: a TreeConnect request's share path is correlated (by session + message id) to the TreeId assigned in its response, and a Create request's filename to the FileId in its response. Later commands that carry that FileId (Close, Read, Write, …) then display the resolved name, e.g. `File: docs\report.docx`. When the establishing packet wasn't captured (so the name is unknown), the FileId is shown as a GUID string instead; a body truncated by `-PacketSize` before the FileId shows `File: ?`. The tables reset at the start of every capture.
...
| QUERY_DIRECTORY (req) | `SMB2 QUERY_DIRECTORY Request, File: … ; FileIdBothDirectoryInformation (0x25), Pattern: *` |
| QUERY_INFO (req) | `SMB2 QUERY_INFO Request, INFO_FILE\FileAllInformation, File: …` |
| CHANGE_NOTIFY (req) | `SMB2 CHANGE_NOTIFY Request, File: … ; Completion Filter: 0x00000017, FILE_NAME, DIR_NAME, ATTRIBUTES, LAST_WRITE` |
| OPLOCK_BREAK (lease) | `SMB2 OPLOCK_BREAK Request, Lease Break: RH -> R; LeaseKey: …` |
| Encrypted | `SMB2 Encrypted, SessId 0x…, len …` |

For an **unresolved** handle the name is replaced by the FileId GUID, e.g. `File: 00001a2b-0000-0000-0000-000000000000`.

`LOGOFF`, `TREE_DISCONNECT`, `CANCEL`, and `ECHO` render as the header line only (no command-specific detail).

### Analysis Details tree

In [Analysis mode](./Analysis-Mode.md) (`-pl Analysis`), selecting an SMB2 packet renders a Wireshark-style detail tree: one collapsible node per message in the (possibly compounded) chain — `SMB2 <Command> - <tree>[; Status: …]` — expanding to the **SMB2 Header** (collapsed by default, summarized as `SMB2 Header - Cmd: <Command> (0x…) <Request|Response>, TID: <tree>[, Status: …]`) and the **command body** (per-command fields: dialects, access/disposition/options and file times for CREATE, offsets/lengths for READ/WRITE, the FileId as a GUID with its resolved name, etc.). For **QUERY_DIRECTORY** the response's directory entries (name, size, attributes, write time, file id) are enumerated, and **QUERY_INFO** / **SET_INFO** decode the file/filesystem info structure — the requested information class is correlated from the request, so both the request and its response must be captured. SYNC and ASYNC headers, the encrypted Transform header, and the legacy SMB1 Negotiate header (with `Flags`/`Flags2`) are each rendered.

**Name-resolution notes.** File/tree names resolve only when both the establishing request and its response were captured (raise `-PacketSize` if Create/TreeConnect bodies are truncated). The name tables are **scoped per TCP connection** (an order-independent hash of the IP 4-tuple), so concurrent connections that reuse the same session/file ids don't collide. Compounded messages that use `SMB2_FLAGS_RELATED_OPERATIONS` (inheriting the previous message's FileId via an all-ones sentinel) resolve to the file established earlier in the same chain. These are display-only aids; the captured pcapng is always exact.
